...
import { Eye, Pin, Star, Trash2, Share2 } from 'lucide-react';
...
const ContextMenu = ({ x, y, onClose, onAction, card }) => {
  ...
  const menuItems = [
...

    {
      ...
      icon: Pin,
      action: 'pin',
      active: card.isPinned,
    },

    {
      label: card.isImportant ? 'Unmark Important' : 'Mark Important',
      icon: Star,
      action: 'important',
      active: card.isImportant,
    },
  ...
  return (
    ...
    >
      {menuItems.map((item, index) => (
        ...
        >
          <item.icon
            className={`w-4 h-4 mr-3 ${item.active ? 'fill-current' : ''}`}
          />
          {item.label}
        </button>
      ))}
